...
public class UserController {
    ...
    public String join(@Valid @ModelAttribute("userJoinForm") UserJoinForm form, BindingResult bindingResult) {
        ...
        return "redirect:/user/login";  // 회원가입이 끝나면 로그인 화면으로 전환
    }
    ...
    // 로그인 처리
    @PostMapping("/login")
    public String login(@Valid @ModelAttribute("userLoginForm") UserLoginForm userLoginForm, 
                        BindingResult bindingResult, 
                        HttpSession httpSession) {
        // BingingResult → 입력 에러 체크
        if (bindingResult.hasErrors()){
            return "user/login";
        }
