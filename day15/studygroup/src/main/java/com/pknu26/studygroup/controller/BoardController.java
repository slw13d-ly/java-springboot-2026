...
public class BoardController {
    ...
    public String detail(@PathVariable("boardId") Long boardId, Model model, HttpSession session) {
    ...
    public String showCreateForm(Model model, HttpSession session) {
        ...
        if (loginUser == null) {
            return "redirect:/user/login"; // 로그인 안한 사람은 게시판 글 못씀
        }
        ...
        // #PRC02 - 입력 값 검증 BoardForm을 읽어온 후, /board/form.html에 model로 전달 
        return "/board/form"; // .html 작성안함. /baord/form.html 화면 띄움
    }

    // 글쓰기 POST
    // #PRC04 - 폼 화면에서 저장버튼 누르면 발생.
    // BoardForm 입력 검증 클래스, BindingResult 검증처리
    @PostMapping("/create") 
    public String create(@Valid BoardForm boardForm, BindingResult bindingResult, HttpSession session) throws Exception {
        ...
        if (bindingResult.hasErrors()) {
            ...
            return "/board/form"; // 500 에러를 위해 잠시 삭제~~ 
        }

        // #PRC06 - 입력 값이 장확하면 Service의 메서드 호출. 파라미터로 폼값
