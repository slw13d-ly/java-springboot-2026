...
public class StudyApplicationService {
    ...
    private final StudyPostMapper studyPostMapper; // 스터디를 신청하려면 스터디 게시글이 필요
    ...
    public void apply(StudyApplicationForm form) {
        ...
        if (!"OPEN".equals(post.getStatus())) { // 스터디포스트 상태가 OPEN이 아니면 신청 불가
    ...
    // 260421. 스터디 신청 제약을 위해 StudyPost도 전달
    public void approve(Long applicationId, StudyPost post) {

        // 스터디포스트 상태가 마감이면 더 이상 승인 불가
    ...
    // 260422. 스터디 거절 후 상태 변경 로직 추가
    public void reject(Long applicationId, StudyPost post) {

        // Long postId = post.getPostId(); // 이거 있으니까 잘 되던뎅?
