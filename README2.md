...
- 구성 3단계
  ...
  - 웹서버(백엔드) - 사용자 요청을 받아서 DB애 데이터 읽고, 프론트엔드에 보낼 데이터를 전송(`응답`)
  - 데이터베이스 - 데이터를 저장, 읽는 부분

- 웹 개념: 시용자의 요청(`Request`)에 대한 서버의 응답(`Response`)
...
- 이전 기술
  - CGI : 옛날 거, 내용 생략
  - Servlet : CGI를 개선한 웹 기술. HTML을 Java 소스 내 전부 작성(개발 난이도 상)
  - EJB(Enterprise Java Bean) : Servlet으로 대형 기업 프로젝트 개발(개발 난이도 극상)
  - `JSP`(Java Server Page) : HTML과 Java 소스를 분리. 쉽게 개발하도록 만들 기술(난이도 중)
    - 개발환경 구성 난이도가 높음
  - `Spring` : Java 개발 전성기. 웹 페이지와 Java 영역 분리. 개발 환경 구성 난이도 줄어듬
    - 개발 환경 구성 난이도 중
    - 대한민국 전자 정부 웹프레임워크 개발
  - `Spring Boot` : Spring 개발 환경 구성 단점, 개발 단점 최소화

#### Spring Boot

- https://spring.io/ 공식 웹사이트
- Spring 의 기술 그대로 사용 (마이그레이션 간단)
- JPA 기술 사용, ERD(매핑)하기 편하고 손쉽게 DB생성, 연동 쉬움
- 개발 시 웹 서버를 따로 구축할 필요 없음 (운영 시에는 설치 필요)
- 단위 테스트용 의존성, 도구 의존성 포함
- 개발을 도와주는 서포트 기능 다수 존재
- 프론트엔드 지원 많음: JSP, Thymeleaf, Mustache 등
- React.js 등 자바스크립트 기반 프론트엔드와 연계 → 풀스택
- MVC(Model-View-Controller) 모델, 각 파트를 따로 개발 가능

![alt text](/day03/image.png)
...
4. Input Group Id
   - 그룹 아이디
   - com.example : mail.naver.com / news.naver.com 등 naver.com을 거꾸로 작성
   - mail, news : 프로젝트 ID
   - `com.pknu26` 로 통일
...
12. Gradle, Java 빌드 진행 - [소스](./day03/demo/src/main/java/com/pknu26/demo/DemoApplication.java)
    - 빌드 이후 작업 표시 줄 `Java: Ready`가 표시되어야 함
    - Java: Error는 프로젝트 생성 실패. 새로 구성

      ![alt text](/day03/image-2.png)

#### Spring Boot 프로젝트 실행

1. Spring Boot Dashboard

   ![alt text](/day03/image-3.png)

2. 컴파일 진행

3. 로그 출력

   ![alt text](/day03/image-4.png)
   - Started DemoApplication... port 8080 확인

4. 웹 브라우저 http://localhost:8080

   ![alt text](/day03/image-5.png)
...
- 어노테이션 목록
  ...
  - @ResponseBody : 응답페이지에 텍스트 출력 어노테이션, 자바의 String 문자열 자동 생성된 웹 페이지에서 렌더링 작업
...
- `src/main/java` : 패키지와 자바 소스가 저장된 폴더
  ...
  - HelloController 클래스 접근 : com.hugo83.spring03.controller.HelloController 로 접근
...
- `src/main/resources` : 자바이외 HTML, CSS, JS, 이미지 파일, 환경파일 등 리소스가 되는 파일 저장하는 폴더
  - static : 정적HTML파일, CSS, JS, 이미지 파일 저장
  - templates : 스프링부트와 연게되는 thymeleaf html이나 mustache 파일 저장
...
- 일반
  - @RequiredArgConstructor : `final` 멤버함수 파라미터를 생성자로 생성하는 Lombok 어노테이션
...
- 게시글 삭제
  ...
  - BoardSevice에 deleteBoardOne 메서드 작업

#### Reply 작업

- entity/Reply 클래스
  - 이전 Board 클래스 생성 시와 동일
  - Board board 멤버변수를 @ManyToOne으로 추가

- entity/Reply 클래스
  - `List<Reply>` replyList 멤버 변수, @OneToMany로 추가
