...
public class CustomOAuth2UserService extends DefaultOAuth2UserService{
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        // DefaultOAuth2UserService를 사용해서 사용자 정보 가져오기
        OAuth2User oauth2User = super.loadUser(userRequest);

        // 로그인 제공사가 누구인지 식별(Goolge, KaKao, Naver ...)
        ...
        if (userSocialAccount != null) { // 이미 소셜계정이 존재
            ...
            if (user == null) {
                ...
                user.setPassword(null); // 소셜 로그인은 패스워드를 직접 처리하지 않음! 

                userMapper.insertSocialuser(user); // 기존 회원가입(join)과 방식이 다름
            }
        ...
        return new DefaultOAuth2User(List.of(new SimpleGrantedAuthority(user.getRole())), attributes, "sub");
    }
}
